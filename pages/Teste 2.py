...
# 2. Aguardar até que o menu seja carregado e clicar no link "Resultado da Pesquisa" correto
resultado_link = wait.until(EC.element_to_be_clickable(
    (By.XPATH, "//a[@class='dropdown-item' and contains(@href, '/admin/pesquisa')]")
))
resultado_link.click()
print("[INFO] Tela 'Resultado da Pesquisa' aberta.")

# 3. Configurar o filtro para exibir "Todos"
...
# 4. Scraping das métricas do Dashboard

# 4.1. Primeira coluna – Métrica (ex: "123")
try:
...
# 4.2. Segunda coluna – Métrica com valor e porcentagem
try:
...
# 4.3. Terceira coluna – Média Geral (calculada a partir das notas das perguntas)
try:
...
# 5. Scraping da tabela "Resultado por PERGUNTAS"

# 5.1. Extração do cabeçalho
try:
...
# 5.2. Extração das linhas (perguntas e suas respectivas notas)
try:
