...
def converter_data(df, col_list):
    """Converte as colunas de data para datetime no formato DD/MM/YYYY."""
    for col in col_list:
...
def parse_month_year(col):
    """
    Identifica colunas no formato 'jan/25', 'fev/25', etc.,
...
def parse_pt_period(s):
    """
    Converte string 'jan/25' em datetime(2025,1,1) para ordenação.
    """
    months_map = {
        'jan': 1, 'fev': 2, 'mar': 3, 'abr': 4, 'mai': 5, 'jun': 6,
        'jul': 7, 'ago': 8, 'set': 9, 'out': 10, 'nov': 11, 'dez': 12
...
def main():
    ...
    # Cálculo de período no departamento (caso precise)
    ...
    with tab_mao_obra:
        ...
        # Identifica colunas mensais (Real)
        monthly_cols_info = []
        ...
        if pd.isna(min_previsao) or pd.isna(max_previsao):
            ...
            if pd.isna(global_min) or pd.isna(global_max) or global_min > global_max:
                st.warning("Intervalo de datas inconsistente.")
            else:
                # Gera range de meses
                ...
                if final_df.empty:
                    ...
                    # Checkbox e multiselect de colunas mensais
                    st.markdown("### Tabela de Distribuição de Custo (Mensal)")
                    if st.checkbox("Exibir Tabela de Distribuição Mensal por Colaborador", value=False):
                        # Identifica colunas mensais
                        monthly_columns = [c for c in df_admin.columns if parse_month_year(c) is not None]
                        if not monthly_columns:
                            st.warning("Não foram encontradas colunas mensais no formato 'xxx/yy'.")
                        else:
                            # Descobre quais meses e anos existem nessas colunas
                            all_month_str = [c[:3].lower() for c in monthly_columns]  # 'jan','fev','mar'...
                            all_year_str  = [c[-2:] for c in monthly_columns]         # '25','26','27'...
                            
                            # Filtra para só anos >= '25'
                            valid_years = sorted({y for y in all_year_str if int(y) >= 25})
                            valid_months = sorted({m for m in all_month_str})
                            
                            # Monta o dicionário para exibir com a 1ª letra maiúscula
                            months_map_display = {
                            ...
                            selected_months = st.multiselect("Selecione o(s) mês(es)", 
                                                             options=[months_map_display[m] for m in valid_months],
                                                             default=[])
                            selected_years  = st.multiselect("Selecione o(s) ano(s)", 
                                                             options=valid_years,
                                                             default=[])
                            
                            if selected_months and selected_years:
                                # Reverte a exibição para 'jan','fev', etc. em minúsculo
                                rev_map = {v:k for k,v in months_map_display.items()}
                                
                                # Gera a lista final de colunas
                                filtered_columns = []
                                for col in monthly_columns:
                                    col_month = col[:3].lower()  # 'jan','fev','mar'...
                                    col_year  = col[-2:]         # '25','26','27'...
                                    if rev_map[col_month.capitalize()] in rev_map:  # já é 'jan','fev'...
                                        # Se col_month está em selected_months e col_year está em selected_years
                                        if (col_month.capitalize() in selected_months) and (col_year in selected_years):
                                            filtered_columns.append(col)
                                
                                filtered_columns = sorted(filtered_columns, key=lambda x: parse_pt_period(x))
                                
                                if filtered_columns:
                                    pivot_table = df_admin.groupby("Colaborador")[filtered_columns].sum()
                                    pivot_table = pivot_table.reindex(columns=filtered_columns)
                                    st.dataframe(pivot_table.style.format("{:,.2f}"), use_container_width=True)
                                else:
                                    st.info("Nenhuma coluna corresponde aos filtros selecionados.")
                            else:
                                st.info("Selecione ao menos um mês e um ano para filtrar a tabela.")
        ...
        if 'final_df' in locals() and not final_df.empty:
            ...
            st.warning("Não há dados para calcular as métricas anuais.")
        ...
        hide_salary = st.checkbox("Ocultar valores de 'Salário Bruto'", value=True)
        if "Colaborador" in df_admin.columns and "Salário Bruto" in df_admin.columns:
            ...
            if hide_salary:
                st.dataframe(df_colab.drop(columns=["Salário Bruto"]).style.format({"Percentual (%)": "{:.2f}%"}))
            else:
        ...
        # Filtro de ANO para contratações futuras (apenas a partir de 2025)
        if not df_admin.empty and "Previsão Data" in df_admin.columns:
            anos_future = sorted([year for year in df_admin["Previsão Data"].dropna().dt.year.unique() if year >= 2025])
            selected_years = st.multiselect("Filtrar por ANO", options=anos_future, default=[])
        else:
            selected_years = []
        df_future = df_admin[df_admin["Admissão"].isna()].copy()
        if selected_years:
            df_future = df_future[df_future["Previsão Data"].dt.year.isin(selected_years)]
        if df_future.empty:
            st.info("Não há contratações futuras previstas (coluna 'Admissão' vazia).")
        else:
            # Agrupa os dados por mês
            df_future["Mes"] = df_future["Previsão Data"].dt.to_period("M")
            meses = sorted(df_future["Mes"].unique(), key=lambda x: x.start_time)
            df_future["Mes_str"] = df_future["Mes"].astype(str)
            ...
            pivot_df = pivot_df.reindex(columns=[str(m) for m in meses])
            fig_heat = px.imshow(
                pivot_df,
                text_auto=True,
                aspect="auto",
                color_continuous_scale="OrRd",
                title="Contratações Futuras - Heatmap (Custo Previsto por Mês)"
            )
