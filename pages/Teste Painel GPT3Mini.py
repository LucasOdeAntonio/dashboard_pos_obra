...
# 7 – Gráfico da aba Departamento: Unidades em Garantia Vigente (acumulado 6 anos)
if "Data Entrega de Obra" in df_dep_renamed.columns and "N° Unidades" in df_dep_renamed.columns:
    # Converter N° Unidades para numérico, se necessário
    df_dep_renamed["N° Unidades"] = pd.to_numeric(df_dep_renamed["N° Unidades"], errors="coerce")
    df_dep_renamed["Ano_Entrega"] = df_dep_renamed["Data Entrega de Obra"].dt.year
    unique_years = sorted(df_dep_renamed["Ano_Entrega"].unique())
    accumulated_data = []
    for year in unique_years:
        # Soma dos valores para os anos entre (year - 5) e year, inclusive
        mask = (df_dep_renamed["Ano_Entrega"] >= (year - 5)) & (df_dep_renamed["Ano_Entrega"] <= year)
        total_unidades = df_dep_renamed.loc[mask, "N° Unidades"].sum()
        accumulated_data.append({"Ano": year, "Total_Unidades": total_unidades})
    df_chart_dep = pd.DataFrame(accumulated_data)
    fig7 = px.bar(
        df_chart_dep,
        x="Ano",
        y="Total_Unidades",
        barmode="stack",
        text="Total_Unidades",
        title="Unidades em Garantia Vigente (Acumulado 6 Anos)"
    )
...
# 13 – MTTC – Tempo Médio de Conclusão (Por Obra)
